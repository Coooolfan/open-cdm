 ...
 */
package com.clougence.clouddm.dsfamily.mysql.language.strategy;
...
import com.clougence.clouddm.dsfamily.mysql.analysis.MySQLParserVisitor;
...
public class MyRdbTablePermissionValidateStrategy extends RdbTablePermissionValidateStrategy {
    ...
    protected List<RuleDomain> resolveDomains(ValidateContext context, MetaService metaService) {
        ...
        for (ValidateStatementState state : context.getStatementStates()) {
            ...
            try {
                DslHelper.doVisitor(context.getDslProvider(), state.getSqlText(), (lexer, parser) -> new MySQLParserVisitor(builder, parser));
