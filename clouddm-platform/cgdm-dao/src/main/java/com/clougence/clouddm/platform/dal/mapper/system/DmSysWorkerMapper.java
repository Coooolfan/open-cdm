 ...
 * @since 1.1.3
 */
public interface DmSysWorkerMapper extends BaseMapper<DmSysWorkerDO> {

    List<DmSysWorkerDO> queryByStateAndClusterId(WorkerState workerState, long clusterId);

    List<DmSysWorkerDO> queryByStateAndScheduleIp(WorkerState workerState, String scheduleIp);

    //    List<WorkerDO> queryByScheduleIp(String scheduleIp);

    List<DmSysWorkerDO> listByCluster(long clusterId);

    List<DmSysWorkerDO> queryByClusterAndIds(long clusterId, List<Long> workerIds);

    List<DmSysWorkerDO> queryByClusters(@Param("clusterIds") List<Long> clusterIds);

    List<DmSysWorkerDO> queryByCondition(Long clusterId, String workerIp);

    List<DmSysWorkerDO> queryByCluster(long clusterId);

    List<DmSysWorkerDO> queryConnectedByClusterId(Long clusterId);

    List<DmSysWorkerDO> queryByConnStatus(WorkerConnStatus connStatus);

    List<DmSysWorkerDO> queryInactiveConnectedByHeartbeatReport(@Param("checkPointEpochMs") Long checkPointEpochMs);

    DmSysWorkerDO getByWsn(String workerSeqNumber);

    DmSysWorkerDO queryConnectedByWsn(String workerSeqNumber);

    DmSysWorkerDO getWorkerIdentityByWsn(String workerSeqNumber);

    DmSysWorkerDO getByName(String workerName);

    /**
     * lock the worker resource
     */
    DmSysWorkerDO lockWorkerByTaskTotalMemory(long workerId, long totalTaskMemMb);
    ...
    int updateWorkerLivenessByWsn(DmSysWorkerDO workerDO);
     ...
     */
    int updateWorkerDynamicStats(ArgSysWorkerObj workerParam);
